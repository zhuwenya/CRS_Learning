...
class Manage_User(object):
    ...
    def add_user(self,user_id):
        # add new user
        if not(self.user_instance.has_key(user_id)):
    ...
    def obtain_user_instance(self,user_id,time):
         ...
         self.add_user(user_id) # add new user(invalid for exsiting user)
    ...
    def clean_user(self):
        ...
        user_active=[k for k, v in self.user_time.iteritems() if v-current_time<-60]
    ...
    def update_user(self,user_id,instance):
        self.user_instance[user_id]=instance
